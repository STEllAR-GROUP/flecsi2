...
context_t::world_comm(std::string name) {
  ...
  flog_assert(!name.empty(), "communicators must hvae valid name");
...
get_self_id() {
  ...
  return ::hpx::threads::get_outer_self_id();
#else
...
create_storage() {
  ...
  if(stg == nullptr) {
    ...
    ::hpx::threads::set_thread_data(
      get_self_id(), reinterpret_cast<std::size_t>(new task_local_data()));
  }
  else {
#if HPX_VERSION_FULL >= 0x011000
    ++stg->count;
#else
    flog_assert(stg == nullptr, "thread local storage should not exist yet");
#endif
  }
}
...
storage() noexcept {
  return reinterpret_cast<task_local_data *>(
    ::hpx::threads::get_thread_data(get_self_id()));
}
...
#endif
  {
    ::hpx::threads::set_thread_data(get_self_id(), 0);
    delete stg;
  }
}
} // namespace flecsi::detail
